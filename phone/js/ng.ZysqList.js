angular.module("myApp", ["ngTouch", "ngq"]).service("Data", function() {
    ...
    if (!e.loading) {
      ...
      console.log(i), console.log($.param(i)), appcan.request.ajax({
        ...
        error: function(i, a, o, n) {
          e.loading = !1, appcan.window.openToast("网络连接不可用", SimcereConfig.ui.toastDuration), t(function() {
            e.itemListErr = !e.itemList.length
          })
        }
      })
  ...
  return function(t) {
     if(t.flowId && t.flowId != 'null'){
         localStorage.setItem("EDZY/ZysqDetail.flowId", t.flowId);
     }else{
         localStorage.removeItem("EDZY/ZysqDetail.flowId");
     }
    return 4 == t.state ? void appcan.window.alert("提示", "无法查看封存的数据", "知道了") : (console.log("patientId: %s", t.id), localStorage.setItem("EDZY/ZysqDetail.state", t.state),localStorage.setItem("EDZY/ZysqDetail.patientId", t.id), localStorage.setItem("EDZY/ZysqDetail.patientName", t.name), void appcan.window.open("EDZY_ZysqDetail", "ZysqDetail.html", 10))
  }
