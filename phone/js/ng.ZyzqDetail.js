angular.module("myApp", ["ngq"]).service("Data", function() {
  ...
  return function() {
    var n = localStorage.getItem("EDZY/ZyzqDetail.ZyzqId");
    console.log("ZyzqId: %s", n);
    var i = {};
    console.log(i), appcan.window.openToast(CR.TOAST_WAITING), appcan.request.ajax({
      type: "GET",
      url: SimcereConfig.server.edzy + "givencyc/" + n,
      data: i,
      ...
      success: function(n, i, c, r, s) {
        if (appcan.window.closeToast(), console.log(n), "0" != n.status) appcan.window.openToast(n.msg, SimcereConfig.ui.toastDuration);
          ...
          l.caseCardFilesView = a(l.caseCardFiles),l.chestCtbFilesView = a(l.chestCtbFiles),l.fupApplyFilesView = a(l.fupApplyFiles), l.aidDrugRecordsFilesView = a(l.aidDrugRecordsFiles), e(function() {
            o.cycle = n.data, o.cycle.begin_end_View = l.beginDate + " ~ " + l.endDate , console.log("detail-----"+JSON.stringify(o)), o.cycle.RegimenName = (!l.combinedTreatName) ? l.theraRegimenName : l.theraRegimenName+"/"+l.combinedTreatName
          });
  ...
  return function(e) {
    ...
    console.log(t), appcan.window.openToast(CR.TOAST_WAITING), appcan.request.ajax({
      type: "POST",
      url: SimcereConfig.server.edzy + "givencyc/cancel",
      data: t,
      dataType: "json",
      timeout: REQUEST_TIMEOUT,
  ...
  return function(e) {
    ...
    console.log(n), appcan.window.openToast(CR.TOAST_WAITING), appcan.request.ajax({
      type: "POST",
      url: SimcereConfig.server.edzy + "givencyc/delete",
      data: n,
      dataType: "json",
      timeout: REQUEST_TIMEOUT,
      success: function(e, o, t, n, a) {
